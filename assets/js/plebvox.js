...
// Static DOM-to-speech character mapping. The speech string is built once and
// every speech character retains the exact originating DOM text node/offset.
(function () {
    'use strict';
    ...
    function installHighlightStyle() {
        ...
        console.log('PlebVox 3.4: static mapping; CSS Highlight API ' + (customHighlightSupported ? 'available' : 'unavailable; DOM fallback enabled'));
    }
    ...
    function isIgnoredElement(tag) {
        return ['SCRIPT','STYLE','NOSCRIPT','IMG','FIGURE','FIGCAPTION','SVG','CANVAS','VIDEO','AUDIO','IFRAME','OBJECT','EMBED','SOURCE','TRACK'].includes(tag);
    }

    function isBlockElement(tag) {
        return ['P','DIV','H1','H2','H3','H4','H5','H6','LI','BLOCKQUOTE','SECTION','ARTICLE','HEADER','FOOTER','MAIN','ASIDE'].includes(tag);
    }

    function isEmojiChar(ch) {
        try { return /[\p{Extended_Pictographic}\p{Emoji_Presentation}\uFE0F\u200D]/u.test(ch); }
        catch (e) { return false; }
    }

    // Build the speech string and its DOM mapping simultaneously.
    // map[i] = { node, offset } for speechText[i]. No later indexOf() search is needed.
    function buildTextMapping(contentNodes) {
        const chars = [];
        const map = [];
        let lastWasSpace = false;

        function appendSpace(node, offset) {
            if (!chars.length || lastWasSpace) return;
            chars.push(' ');
            map.push({node:node, offset:offset});
            lastWasSpace = true;
        }
        ...
        function appendText(node) {
            ...
            for (let i = 0; i < raw.length; i++) {
                ...
                if (/\s/.test(ch)) { appendSpace(node, i); continue; }
                chars.push(ch);
        ...
        function process(node) {
            ...
            if (isIgnoredElement(tag)) return;
            if (isBlockElement(tag)) appendSpace(null, 0);
        ...
        for (let i = 0; i < speechText.length; i++) {
            ...
            if (!entry || entry.node !== m.node || entry.speechEnd !== i || entry.rawEnd !== m.offset) {
                entry = {node:m.node, rawText:m.node.textContent || '', speechStart:i, speechEnd:i + 1, rawStart:m.offset, rawEnd:m.offset + 1, charMap:[]};
                mapping.push(entry);
            } else {
                entry.speechEnd = i + 1;
    ...
    function rebuildSectionMapping(section) {
        ...
        section.contentNodes = collectSectionNodes(section.startNode, section.endNode);
    ...
    function getPlebVoxSections() {
        ...
        markers.forEach(function(m) {
            if (m.type === 'PLEBVOX:START') { if (!startNode) startNode = m.node; else console.warn('PlebVox: nested START marker'); return; }
    ...
    function findDomPosition(section, charIndex) {
        const map = section && section.mappingData && section.mappingData.charMap || [];
        if (!map.length) return null;
        let i = Math.max(0, Math.min(charIndex, map.length - 1));
    ...
    function highlightWordByCharIndex(charIndex, section) {
        ...
        if (!bounds) return;
        const first = findDomPosition(section, bounds.start);
        const last = findDomPosition(section, bounds.end - 1);
        if (!first || !last || !first.node || !last.node) return;
        ...
        if (first.node === last.node) {
            const node=first.node, p=node.parentNode, raw=node.textContent||'';
            const before=document.createTextNode(raw.slice(0,first.offset));
            const word=document.createTextNode(raw.slice(first.offset,last.offset+1));
            const after=document.createTextNode(raw.slice(last.offset+1));
            const span=document.createElement('span'); span.className='plebvox-highlight'; span.appendChild(word);
            if(before.nodeValue)p.insertBefore(before,node); p.insertBefore(span,node); if(after.nodeValue)p.insertBefore(after,node); p.removeChild(node);
            rebuildSectionMapping(section);
        } else {
            console.warn('PlebVox: cross-node word encountered; CSS Highlight API unavailable');
        }
    }
    ...
    function speakSection(text, section, controls) {
        ...
        currentSectionData=section; activeControls=controls;
        utterance=new SpeechSynthesisUtterance(speechText); utterance.rate=speechRate; utterance.pitch=1; utterance.volume=1;
        waitForVoices().then(function(voices){
            availableVoices=voices;
            if(voices.length){ voicesLoaded=true; voiceLoadComplete=true; updateAllVoiceSelectors(); updateVoiceLoadStatus();
                if(!selectedVoice || !voices.includes(selectedVoice)) selectedVoice=voices.find(v=>/^en[-_]ZA$/i.test(v.lang))||voices.find(v=>/^en[-_]GB$/i.test(v.lang))||voices.find(v=>/^en[-_]AU$/i.test(v.lang))||voices.find(v=>/^en/i.test(v.lang))||voices[0];
                if(selectedVoice){utterance.voice=selectedVoice;if(selectedVoice.lang)utterance.lang=selectedVoice.lang;}
            }
            utterance.onstart=function(){isReading=true;isPaused=false;clearHighlights();updateControls(controls,'playing');};
            utterance.onboundary=function(event){if(typeof event.charIndex==='number'&&event.charIndex>=0)highlightWordByCharIndex(event.charIndex,section);};
    ...
    function waitForVoices(){
        ...
        voicePromise=new Promise(function(resolve){
            ...
            let voices=window.speechSynthesis.getVoices(); if(voices.length){resolve(voices);return;}
            ...
            setTimeout(function(){voices=window.speechSynthesis.getVoices();if(voices.length){window.speechSynthesis.removeEventListener('voiceschanged',changed);resolve(voices);return;}setTimeout(function(){window.speechSynthesis.removeEventListener('voiceschanged',changed);resolve(window.speechSynthesis.getVoices());},1500);},500);
        });
        return voicePromise;
    }
    function loadVoices(){if(voicesLoaded)return;waitForVoices().then(function(v){availableVoices=v;voiceLoadComplete=true;if(v.length){selectedVoice=v.find(x=>/^en[-_]ZA$/i.test(x.lang))||v.find(x=>/^en[-_]GB$/i.test(x.lang))||v.find(x=>/^en/i.test(x.lang))||v[0];voicesLoaded=true;}updateAllVoiceSelectors();updateVoiceLoadStatus();}).catch(function(e){console.warn('PlebVox: voice loading error',e);voiceLoadComplete=true;updateVoiceLoadStatus();});}
    ...
    function populateVoiceSelector(select){const current=select.value;select.innerHTML='';if(!availableVoices.length){const o=document.createElement('option');o.value='';o.textContent='No voices available';select.appendChild(o);return;}availableVoices.forEach(function(v){const o=document.createElement('option');o.value=v.name;let f='🌐';if(/^en[-_]GB$/i.test(v.lang))f='🇬🇧';else if(/^en[-_]US$/i.test(v.lang))f='🇺🇸';else if(/^en[-_]AU$/i.test(v.lang))f='🇦🇺';else if(/^en[-_]ZA$/i.test(v.lang))f='🇿🇦';o.textContent=f+' '+v.name;select.appendChild(o);});if(current&&availableVoices.some(v=>v.name===current))select.value=current;else if(selectedVoice)select.value=selectedVoice.name;}

    function createSectionControls(section,index){
        const container=document.createElement('div');container.className='plebvox-control-'+index;container.setAttribute('data-plebvox-ui','true');container.setAttribute('aria-label','PlebVox controls for Part '+section.number);container.style.cssText='margin:.75rem 0;padding:.75rem 1rem;max-width:100%;background:var(--secondary-nav-bg,#f4f4f4);border-radius:8px;border:1px solid var(--border,#ddd);text-align:center;';
        ...
        const bc=document.createElement('div');bc.style.cssText='display:flex;justify-content:center;align-items:center;gap:.5rem;flex-wrap:wrap;margin-bottom:.5rem;';
        ...
        play.addEventListener('click',()=>speakSection(section.text,section,controls));pause.addEventListener('click',()=>pauseSpeech(controls));resume.addEventListener('click',()=>resumeSpeech(controls));stop.addEventListener('click',()=>stopSpeech(controls));bc.append(play,pause,resume,stop,status);
        ...
        const sid=generateUniqueId('plebvox-speed'),sl=document.createElement('label');sl.textContent='Speed:';sl.setAttribute('for',sid);const speed=document.createElement('input');speed.id=sid;speed.type='range';speed.min='0.5';speed.max='2';speed.step='0.1';speed.value=speechRate;speed.setAttribute('aria-label','Reading speed');speed.addEventListener('input',function(){speechRate=parseFloat(this.value);});
        const vid=generateUniqueId('plebvox-voice'),vl=document.createElement('label');vl.textContent='Voice:';vl.setAttribute('for',vid);const vs=document.createElement('select');vs.id=vid;vs.className='plebvox-voice-select';vs.setAttribute('aria-label','Select reading voice');vs.style.cssText='max-width:240px;padding:.25rem;';vs.addEventListener('change',function(){const found=availableVoices.find(v=>v.name===this.value);if(found)selectedVoice=found;});
        const statusVoice=document.createElement('div');statusVoice.className='plebvox-voice-status';statusVoice.setAttribute('aria-live','polite');statusVoice.style.cssText='font-size:.75rem;margin-top:.35rem;';row.append(sl,speed,vl,vs);container.append(title,bc,row,statusVoice);populateVoiceSelector(vs);return{element:container,playBtn:play,pauseBtn:pause,resumeBtn:resume,stopBtn:stop,statusText:status};
    }

    function initPlebVox(){if(!('speechSynthesis'in window)){console.warn('PlebVox: speech synthesis unavailable');return;}installHighlightStyle();sectionDataList=getPlebVoxSections();if(!sectionDataList.length)return;sectionDataList.forEach(function(section,index){const controls=createSectionControls(section,index);section.controlElement=controls.element;section.startNode.parentNode.insertBefore(controls.element,section.startNode.nextSibling);});loadVoices();console.log('PlebVox 3.4: initialized with static character mapping');}
